...
После добавления получившейся транзакции в чейн, мы можем узнать идентификатор нашего нового контракта. В нашем случае это `1.16.1337`.
Теперь, мы можем узнать текущий баланс контракта, а также вызвать его методы.


Вызовем метод, возвращающий константную строку - `greet()`.
```
call_contract nathan 1.16.1337 "cfae3217" 0 ECHO 1 2000000 true true
```

Вызовем метод, производящий трансфер на наш аккаунт и создающий ивент - `getPennie()`.
```
call_contract nathan 1.16.1337 "9e19b52d" 0 ECHO 1 2000000 true true
```

Вызовем метод, возвращающий все деньги контракта обратно нам и уничтожающий сам контракт - `breakPiggy()`.
```
call_contract nathan 1.16.1337 "0314c1bf" 0 ECHO 1 2000000 true true
```
