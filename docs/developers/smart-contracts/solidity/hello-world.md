...
Пример ниже описывает механизм создания, деплоя и вызова контракта в сети Echo, написанного на Solidity. Для взаимодействия с блокчейном будет использован **Echo cli-wallet**. Есть и другие варианты создания контракта, такие как использование библиотек на js или python, либо загрузка контракта через десктопный кошелек вручную. Примеры для этих методов вы сможете найти в документациях соответствующих библиотек или приложений.

Для примера мы будем использовать контракт `piggy.sol`. Этот контракт принимает деньги при создании и выводит их по единице либо все разом, но при этом удаляя себя.
...
Ниже мы воспользуемся solc, чтобы скомпилировать контракт и вывести получившийся байткод и хэши функций, но хеши и байткод может быть получен и иными способами - используя онлайн IDE, как пример.
...
Создаем контракт от имени nathan с помощью команды `create_contract`.
