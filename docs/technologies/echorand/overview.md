...
EchoRand is based on the concept of verifiable randomness generated in distributed way and accompanied by the role system. There are three roles specified: producer, verifier and acceptor. Producer is responsible for construction of a new block. Verifier is in its turn responsible for validation and approval of the proposed block. Finally, acceptor is a passive role, it accepts the approved block and adds it to the end of its own local instance of distributed ledger. EchoRand performs in rounds. At each round new sets of producers and verifiers are selected from all users of network in such a way that:

- Distribution of roles for the round is not known to each user in the network before the round start.
- Distribution of roles for the round is known to each user in the network on round start without any explicit message exchange required.
- Distribution of roles for the any future round can not be predicted by any of the network’s users.

The size of pools of producers and verifiers is globally-known and configurable. It can be dynamically adjusted to reach the best trade off between security and performance.
...
As a safeguard against possible Sybil attacks, each user’s probability of becoming a verifier or a producer for the round is directly proportional to that user's account balance.
