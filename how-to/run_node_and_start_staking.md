...
> Если вы не хотите поднимать свою полную ноду, но хотите принимать участие в консенсусе через делигирование, вы можете подключить echo_wallet к уже существующим публичным нодам, которые учавствуют в блокчейне. Эти публичные ноды запущены как предприятиями, так и частными лицами.
...
> Для того, чтобы начать учавствовать в staking, у вас должен быть зарегистрирован аккаунт в блокчейне. И на нём необходимо иметь баланс в ECHO. Документ по регистрации аккаунта можно найти [здесь](/how-to/register-account.md#Register_account).

После всех этих действий вы можете начать стейкать. Для этого вам необходимо заморозить ECHO на определённый срок. Чтобы посмотреть на сколько дней вы можете заморозить свои средства и какой будет повышающий коэффициент,для этого вызовите `get_global_properties` и найдите поле `frozen_balances_multipliers` в полученном объекте. Первое число в мапе показывает на сколько вы можете заморозить свои средства, второе - какой повышающий коэффициент будет при такой заморозке(умноженный на 100).
...
Делегатом аккаунта `1.2.503` из примекра выше является аккаунт `1.2.8`.
