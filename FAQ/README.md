...
* [Bitcoin and Ethereum Sidechains](#bitcoin-and-ethereum-sidechains)
  ...
  * [How do Echo sidechains work?](#how-do-echo-sidechains-work)
  ...
  * [How do Echo blockchain compare with other close and similar blockchain projects such as EOS, Cosmos, Polkadot and Algorand?](#how-do-echo-blockchain-compare-with-other-close-and-similar-blockchain-projects-such-as-eos-cosmos-polkadot-and-algorand)
  * [How do Echo compare with other projects such as Rootstock, Liquid and Lightning Network?](#how-do-echo-compare-with-other-projects-such-as-rootstock-liquid-and-lightning-network)
...
* [Network Security](#network-security)
  ...
  * [How does Echo secure pegged sidechain assets?](#how-does-echo-secure-pegged-sidechain-assets)
...
A sidechain is a method of representing an asset (such as Bitcoin) from it's native blockchain onto a new blockchain infrastructure. A sidechain is typically created through a 2-way peg, where some assets are locked on e original chain before the corresponding assets are created on the destination chain. Examples of other sidechains include Liquid and RSK for Bitcoin and POA Network for Ethereum.

### How do Echo sidechains work?

Similar to other production sidechains, Echo's sidechains work through a 2-way peg to the native blockchain. For Bitcoin, this means that BTC is deposited into a multisig address and a corresponding amount of Echo BTC, or eBTC, is minted on the Echo blockchain to the user who initiated the deposit. The new eBTC can be user to interact with a smart contract, traded on a decentralized exchange, or transferred to another user. When the eBTC needs to be converted back to BTC, it can be locked into a contract and the corresponding amount of BTC will be released on the bitcoin chain. The protocol ensures that the same BTC cannot be valid on both blockchains at the same time.
...
Yes. Echo supports all ERC-20 standard Ethereum assets and allows them to moved on and off Echo through the Ethereum sidechain.
...
### How do Echo blockchain compare with other close and similar blockchain projects such as EOS, Cosmos, Polkadot and Algorand?
...
### How do Echo compare with other projects such as Rootstock, Liquid and Lightning Network?
...
Liquid can be seen as the second layer for the Bitcoin network, which enables fast yet secure Bitcoin transfers.
...
In the case of RSK or Liquid, we are expanding the functionality of Bitcoin, but are still locked within the network. Echo, on the other hand, makes it possible to go beyond the Bitcoin network and implement business logic, which will include the interaction of several blockchains and cryptocurrencies.
Echo is suitable for large scale payments, whist Lightning Network is designed to handle small to medium-sized transactions.
In addition, the Lightning Network has the main problem of centralization of payment hubs, which is quite similar to miner centralization, whereas Echo Network has minimal risks of centralization of hubs in accordance with the PoWR consensus and EchoRand mechanism. Plus all transactions are tracked by the Echo's main channel, that eliminating privacy issues that arise in the Lightning Network.
...
- Echo Network is a fully cross-chain interoperable ecosystem with the ability to share, see, and access information across different networks without needing for intermediaries.
- Echo supports Bitcoin and Ethereum sidechains with enabling interoperability of the two networks within an innovative ecosystem for skyrocket DeFi where each node are involved in decision-making process proportional to the amount of cryptocurrency it holds.
- Echo proposes high resistance to blockchain forks and reorganizations, and strength to any malicious actions by any network actors, including failure to propagate messages and sending contradictory messages
...
Echo's overlay network is the infrastructure and developer platform necessary to create and deploy high-performing and user-friendly decentralized applications that build on top of leading protocols.Echo supports honest partnerships in the blockchain industry and doesn't act like a tough competitor in relation to similar blockchain projects.
Echo uses industry best practices and approaches to provide the community with a new solution and solve existing problems or disadvantages of other blockchain products for achieving key industry goals in blockchain security and performance.
...
Transaction fees can be paid in many different currencies and are converted into ECHO by the protocol’s underlying fee pools. Additionally, decentralized applications in Echo can pay for their own execution, so that users aren’t required to individually pay transaction fees.
Get started here.

### How can people stay up to date on Echo?

People may stay up to date on Echo by joining to Echo Community or following Echo in social media
