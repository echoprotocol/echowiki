...
- Field `address` of `eth_getLogs`method in ethrpc to string instead of list.
- `object_type` in variant now support `null` for passing in ethrpc.
- Method `get_block` with `0` in argument return genesis block.
- Method `eth_chainId` in ethrpc return uint_t instead of hash_t.

### Fixed

- Calling method `generate_account_address` without import private_key return `No required keys found in potential signature keys. You should import account first.` assert.
- `cumulativeGasUsed` and `gasUsed` have not zero value in `eth_getTransactionReceipt` method in ethrpc. 
