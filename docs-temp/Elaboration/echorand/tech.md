...
| **B<sub>r</sub>** | блок, созданный на раунде **r**, равен { **r**, **producer-id**, **PAY<sup>r</sup>**, **Q<sub>r</sub>**, **HB<sub>r-1</sub>** } |
...
Вычисляется набор исполнителей **A<sub>s</sub> = VRF(r,s) = {i<sub>1</sub>, i<sub>2</sub>, ...}** и **N<sub>s</sub> = VRFN(r,s)**, тут **s = 1,...,7**.

Где:
* **A<sub>s</sub>** - набор **всех** исполнителей, активных на шаге **s** (**a**ctive).
* **N<sub>s</sub>** - набор исполнителей, активных на шаге **s** на текущем узле (**n**ode).

Расчет исполнителей ведется для всех шагов этапа `gc` и первой итерации этапа `bba`.

При необходимости последующих итераций алгоритма `bba`, значение функции **VRF(r,s)** будет дополнительно вычисляться перед стартом итераций, сразу для всех трех шагов `bba`.
...
> Пусть некоторый блок недоступен и этот блок не содержит транзакций (нулевой/пустой блок),
> а сертификат этого блока доступен.
>
> Тогда из сертификата можно определить, что блок пуст. Для пустого блока **Q<sub>r</sub>** = **H( Q<sub>r-1</sub>, r )**, что легко проверить зная **Q<sub>r-1</sub>**.
> Следовательно, такой пустой блок для построения корректного **PAY<sup>r</sup>** не нужен. Значит во входных данных алгоритма необходимо иметь все предыдущие блоки, содержащие непустой набор транзакций.
...
| **block** | блок |
| **signature** | подпись блока ключом исполнителя **id** |
...
|---|---|
| **round** | текущий раунд |
| **id** | идентификатор исполнителя, создавшего сообщение и блок |
| **rand** | подпись случайного вектора предыдущего раунда ключом исполнителя **id** |
...
|---|---|
| **round** | текущий раунд |
| **step** | шаг алгоритма, **s = 5,...** |
| **id** | идентификатор исполнителя, создавшего данное сообщение |
| **value** | оценка в рамках алгоритма **BBA**, 0 или 1 |
| **block_hash** | хеш выбранного блока |
| **leader** | идентификатор выбранного лидера, который создал блок |
| **signature** | подпись полей **value**, **block_hash**, **leader** |
...
**TODO** - на основе [algorand-v9][] (9. Handling Forks, page 70)
